  ...
  obtenerDatosParaValidacion().then((datos) => {
    ...
    datos.forEach((persona) => {
      if (persona.idexperimento) {
        conteo[persona.idexperimento] =
          (conteo[persona.idexperimento] || 0) + 1;
      }
    });

    const experimentosBanos = [
      "SCBI1",
      "SCBI2",
    ...
    let experimentosDisponibles;
    if(formData.distrito==="Cajamarca"){
      experimentosDisponibles = experimentosCajamarca.filter((experimento) => {
        return (conteo[experimento] || 0) < 10; // Si no está en el conteo, tiene 0 asignaciones
      });
    } else {
       experimentosDisponibles = experimentosBanos.filter((experimento) => {
        return (conteo[experimento] || 0) < 10; // Si no está en el conteo, tiene 0 asignaciones
      });
    }
    ...
    datos.forEach((persona) => {
      if (persona.fechaformulario) {
        conteoFechas[persona.fechaformulario] =
          (conteoFechas[persona.fechaformulario] || 0) + 1;
      }
    });
    ...
    const fechaAleatoria =
      fechasDisponibles[Math.floor(Math.random() * fechasDisponibles.length)];
...
function validarCuotas(edad, genero, distrito, datos) {
  ...
  const personasEnRango = datos.filter((persona) => {
    return (
      persona.edad >= rangoEdad.edad_min &&
      persona.edad <= rangoEdad.edad_max &&
      persona.sexo === genero &&
  ...
  switch (e.target.id) {
    ...
    case "edad":
      ...
      if (Number.isInteger(edad) && edad >= 20) {
        document
          .getElementById("grupo__edad")
          .classList.remove("formulario__grupo-incorrecto");
        document
          .getElementById("grupo__edad")
          .classList.add("formulario__grupo-correcto");
        document
          .querySelector("#grupo__edad i")
          .classList.add("fa-check-circle");
        document
          .querySelector("#grupo__edad i")
          .classList.remove("fa-times-circle");
        document
          .querySelector("#grupo__edad .formulario__input-error")
